...
// The gap block is three bullets, one per question the reader is asking: what do
// I already have, what does the next level ask, where is my room. Capped per
// bullet rather than per block — a long bullet stops being scannable, which is
// the only reason to prefer bullets over the paragraph this used to be.
const GAP_POINTS = 3;
...
gap_points — tepat tiga poin. Satu poin maksimum ${GAP_POINT_WORD_CAP} kata, keras.
...
Pakai bahasa sederhana. Kata sehari-hari, bukan kata kantor. Satu poin satu gagasan, dan poin itu harus bisa dibaca sendirian tanpa poin sebelumnya. Jangan memotong satu paragraf menjadi tiga. Jangan menulis tanda "-" atau angka di depan poin, halaman yang menambahkannya.
...
Poin dua menyebut apa yang diminta level berikutnya. Levelnya yang menuntut, bukan orangnya yang kurang. Tulis "level berikutnya minta cakupan yang lebih luas", bukan "kamu kurang cakupan".

Poin tiga membahas altitude, dan hanya jarak antara levelnya dengan level di atasnya. Sampaikan gagasannya dengan kata-katamu sendiri:
...
/**
 * The three bullets as one block of prose, for the checks that are about the
