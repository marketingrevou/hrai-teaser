...
// The gap block is one paragraph of two sentences: what do I already have, and
// what does the next level ask. The third beat — where my room is — rides in the
// second sentence rather than being dropped, because altitude is otherwise the one
// quiz answer that reaches no copy at all (ai_body reads ai_level only) while
// cacheKey still keys on it, which would spend generations on variants that cannot
// differ. Capped per block, not per sentence: prose is read, not scanned, and
// tone.js already holds any single sentence to 24 words.
const GAP_WORD_CAP = 40;
...
gap_body — satu paragraf, dua kalimat. Maksimum ${GAP_WORD_CAP} kata, keras.
...
Pakai bahasa sederhana. Kata sehari-hari, bukan kata kantor. Tulis satu paragraf yang mengalir: kalimat kedua menyambung kalimat pertama, bukan berdiri sendiri. Jangan menulis daftar. Jangan menulis tanda "-", angka, atau baris baru di dalam teks.

Kalimat pertama menyebut apa yang sudah dia punya. Sebut spesifik pakai role_fn atau role_type. Jangan pernah membuka dengan kekurangan.

Kalimat kedua menyebut apa yang diminta level berikutnya, dan di klausa yang sama menyebut di mana ruangnya. Levelnya yang menuntut, bukan orangnya yang kurang. Tulis "level berikutnya minta cakupan yang lebih luas", bukan "kamu kurang cakupan". Yang diminta dulu, lalu ruangnya, disambung "dan", "karena", atau "sejak".
...
role_type menentukan kalimat kedua:
...
Satu paragraf, bahasa sederhana, dan langsung menjawab judulnya: apa yang berubah untuk dia karena AI. Bahas ai_level saja. Jangan mengulang apa pun dari gap_body. Sampaikan gagasannya dengan kata-katamu sendiri:
...
1. Kata "kamu" ada di ai_body, dan ada di gap_body minimal sekali.
...
7. gap_body di bawah ${GAP_WORD_CAP} kata, dan ai_body di bawah ${AI_WORD_CAP} kata. Keduanya satu paragraf, bukan daftar.
  ...
  json_schema: {
    ...
    schema: {
      ...
      properties: {
        gap_body: {
          type: 'string',
          description: `Blok B. Satu paragraf, dua kalimat, maksimum ${GAP_WORD_CAP} kata.`,
        },
        ai_body: {
          type: 'string',
...
export function validate({ gap_body, ai_body }) {
  ...
  // The block renders as one paragraph, so a list the model smuggled in as markup
  // would show up as literal dashes mid-sentence.
  if (/(^|\n)\s*([-*•]|\d+[.)])\s/.test(gap_body)) found.push('gap_body written as a list');
  if (wordCount(ai_body) > AI_WORD_CAP) found.push(`ai_body over ${AI_WORD_CAP} words`);
  for (const [pattern, label] of VIOLATIONS) {
    if (pattern.test(gapText)) found.push(`gap_body: ${label}`);
  ...
  for (const label of hardViolations(gapText)) found.push(`gap_body: ${label}`);
